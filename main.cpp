...
int main(){
	...
	posCache cache[dimCache];
	for(i=0;i<dimCache;i++){
		posCache c(dimBlocco);
		cache[i]=c;
	}
	...
	switch(scelta){
		...
		case 1:
			...
			while(scelta!=6){
				...
				if(scelta==1){
					...
					if(cache[pos%dimCache].libera==false){
						colora(12);
						...
						for(j=0;j<dimBlocco;j++)
							cache[pos%dimCache].dati[j]=memoria[pos][j];

						cache[pos%dimCache].libera=false;

						cache[pos%dimCache].contFIFO=0;
						cache[pos%dimCache].contLRU=0;
					...
					for(i=0;i<dimCache;i++){
						if(cache[i].libera==false)
							cout<<" FIFO: "<<cache[i].contFIFO<<" ; LRU: "<<cache[i].contLRU<<" --> ";
						else
							cout<<"\t\t      ";
						cout<<i<<". | ";
					...
					if(cache[blocco].libera==true){
						...
						cout<<endl<<"\t\t**** BLOCCO DI CACHE "<<blocco<<" ****"<<endl<<endl;
							cout<<" FIFO: "<<cache[blocco].contFIFO<<" ; LRU: "<<cache[blocco].contLRU<<" --> ";
						cout<<blocco<<". | ";
						for(j=0;j<dimBlocco;j++)
							cout<<cache[blocco].dati[j]<<" | ";

						cache[blocco].contLRU=0;
				...
				if(scelta==1){
					...
					if(posLibere==0) {
						...
						if(scelta==1){
							...
							for(j=0;j<dimBlocco;j++)
								cache[casuale].dati[j]=memoria[pos][j];

							cache[casuale].libera=false;
							cache[casuale].contFIFO=0;
							cache[casuale].contLRU=0;
							...
							int maxIndice=-1;
							for(i=0;i<dimCache;i++)
								if(cache[i].contFIFO > maxFIFO){
									maxFIFO = cache[i].contFIFO;
									maxIndice = i;
								}

							cout<<"Sto caricando il blocco di memoria nella posizione di cache "<<maxIndice<<" (blocco CARICATO meno recentemente) ..."<<endl;
							for(j=0;j<dimBlocco;j++)
								cache[maxIndice].dati[j]=memoria[pos][j];

							cache[maxIndice].libera=false;
							cache[maxIndice].contFIFO=0;
							cache[maxIndice].contLRU=0;
							...
							int maxIndice=-1;
							for(i=0;i<dimCache;i++)
								if(cache[i].contLRU > maxLRU){
									maxLRU = cache[i].contLRU;
									maxIndice= i;
								}
						...
						while(cache[posizioneCache].libera==false)
							posizioneCache++;

						cout<<"Carico il blocco di memoria scelto nella posizione di cache "<<posizioneCache<<" (la prima disponibile) ..."<<endl<<endl;

						for(j=0;j<dimBlocco;j++)
								cache[posizioneCache].dati[j]=memoria[pos][j];

						cache[posizioneCache].libera=false;
						cache[posizioneCache].contFIFO=0;
						cache[posizioneCache].contLRU=0;
		...
		case 3:
			int nGruppi;
				...
				cin>>nGruppi;
				while(nGruppi<2 || dimCache%nGruppi!=0){
					if(nGruppi<2){
						colora(12);
						cout<<"Numero di gruppi non valido! Ci devono essere almeno 2 gruppi!"<<endl;
						colora(7);
						cout<<"Inserisci un nuovo numero di gruppi : ";
						cin>>nGruppi;
					}
					else{
						colora(12);
